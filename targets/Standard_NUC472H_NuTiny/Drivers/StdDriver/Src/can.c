...
{
    STR_CANMSG_T rMsg;
    CAN_ReadMsgObj(tCAN, u32MsgNum,TRUE_nuc, &rMsg);
...
int32_t CAN_SetRxMsg(CAN_T *tCAN, uint32_t u32MsgNum , uint32_t u32IDType, uint32_t u32ID)
{
    ...
    while(CAN_SetRxMsgObj(tCAN, u32MsgNum, u32IDType, u32ID, TRUE_nuc) == FALSE_nuc) {
        u32TimeOutCount++;

        if(u32TimeOutCount >= 0x10000000) return FALSE_nuc;
    }
...
int32_t CAN_SetMultiRxMsg(CAN_T *tCAN, uint32_t u32MsgNum , uint32_t u32MsgCount, uint32_t u32IDType, uint32_t u32ID)
{
    ...
    for(i= 1; i < u32MsgCount; i++) {
        ...
        while(CAN_SetRxMsgObj(tCAN, u32MsgNum, u32IDType, u32ID, u32EOB_Flag) == FALSE_nuc) {
            u32TimeOutCount++;
...
int32_t CAN_Transmit(CAN_T *tCAN, uint32_t u32MsgNum , STR_CANMSG_T* pCanMsg)
{
    if((tCAN->CON & CAN_CON_TEST_Msk) && (tCAN->TEST & CAN_TEST_BASIC_Msk)) {
        ...
        if(CAN_SetTxMsg(tCAN, u32MsgNum, pCanMsg) == FALSE_nuc)
            return FALSE_nuc;
...
int32_t CAN_Receive(CAN_T *tCAN, uint32_t u32MsgNum , STR_CANMSG_T* pCanMsg)
{
    if((tCAN->CON & CAN_CON_TEST_Msk) && (tCAN->TEST & CAN_TEST_BASIC_Msk)) {
        ...
        return CAN_ReadMsgObj(tCAN, u32MsgNum, TRUE_nuc, pCanMsg);
    }
}
