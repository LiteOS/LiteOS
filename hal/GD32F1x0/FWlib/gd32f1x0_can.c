...
ErrStatus can_wakeup(uint32_t can_periph)
{
    ...
    while((CAN_CTL_SLPWMOD == (CAN_CTL(can_periph) & CAN_CTL_SLPWMOD)) && (timeout != 0x00)){
        timeout--;
    }
    if(CAN_STAT_SLPWS != (CAN_STAT(can_periph) & CAN_STAT_SLPWS)){
        flag = ERROR;
...
{  
    ...
    /* error flags in ERR */
    if((uint32_t)RESET != (can_flag & CAN_FLAGS_ERR)){
       if((uint32_t)RESET != (CAN_ERR(can_periph) & (can_flag & CAN_FLAG_MASK))){
           val = SET;
       }else{
           val = RESET;
       }
       /* working mode flags in STAT */
   }else if((uint32_t)RESET != (can_flag & CAN_FLAGS_STAT)){
       if((uint32_t)RESET != (CAN_STAT(can_periph) & (can_flag & CAN_FLAG_MASK))){
           val = SET;
       }else{
           val = RESET;
       }
       /* transmit flags in TSTAT */
   }else if((uint32_t)RESET != (can_flag & CAN_FLAGS_TSTAT)){
       if((uint32_t)RESET != (CAN_TSTAT(can_periph) & (can_flag & CAN_FLAG_MASK))){
           val = SET;
       }else{
           val = RESET;
       }
       /* receive flags in REFIFO0 */
   }else if((uint32_t)RESET != (can_flag & CAN_FLAGS_RFIFO0)){
       if((uint32_t)RESET != (CAN_RFIFO0(can_periph) & (can_flag & CAN_FLAG_MASK))){
           val = SET;
       }else{
           val = RESET;
       }
       /* receive flags in REFIFO1 */
   }else if((uint32_t)RESET != (can_flag & CAN_FLAGS_RFIFO1)){
       if((uint32_t)RESET != (CAN_RFIFO1(can_periph) & (can_flag & CAN_FLAG_MASK))){
           val = SET;
       }else{
           val = RESET;
       }
...
void can_flag_clear(uint32_t can_periph, uint32_t can_flag)
{
    ...
    /* error flags in ERR */
    if(CAN_FLAG_ERRN == can_flag){
        /* clear the selected CAN flags */
        CAN_ERR(can_periph) = RESET;
    }else{
        /* STAT or TSTAT or RFIFO0 or RFIFO1 */
        val = can_flag & CAN_FLAG_MASK;
        if((uint32_t)RESET != (can_flag & CAN_FLAGS_RFIFO0)){
            /* receive flags */
            CAN_RFIFO0(can_periph) = val;
        }else if((uint32_t)RESET != (can_flag & CAN_FLAGS_RFIFO1)){
            /* receive flags */
            CAN_RFIFO1(can_periph) = val;
        }else if((uint32_t)RESET != (can_flag & CAN_FLAGS_TSTAT)){
            /* transmit flags */
            CAN_TSTAT(can_periph) = val;
        }else{
            /* working mode flags */
            CAN_STAT(can_periph) = val;
        }
     }
}
...
/*!
    \brief      get CAN interrupt flag
     \param[in]  can_periph
      \arg        CANx(x=0,1)
    \param[in]  can_interrupt_flag
      \arg        CAN_INT_TME: transmit mailbox empty interrupt
      \arg        CAN_INT_RFNE0: FIFO0 not empty interrupt
      \arg        CAN_INT_RFF0: FIFO0 full interrupt
      \arg        CAN_INT_RFO0: FIFO0 overrun interrupt
      \arg        CAN_INT_RFNE1: FIFO1 not empty interrupt
      \arg        CAN_INT_RFF1: FIFO1 full interrupt
      \arg        CAN_INT_RFO1: FIFO1 overfull interrupt
      \arg        CAN_INT_WERR: warning error interrupt
      \arg        CAN_INT_PERR: passive error interrupt
      \arg        CAN_INT_BO: bus-off error interrupt
      \arg        CAN_INT_ERRN: error number interrupt
      \arg        CAN_INT_ERR: error interrupt
      \arg        CAN_INT_WU: wake-up interrupt
      \arg        CAN_INT_SLPW: sleep working interrupt interrupt
    \param[out] none
    \retval     FlagStatus
*/
FlagStatus can_interrupt_flag_get(uint32_t can_periph, uint32_t can_interrupt_flag)
{
    ...
    if((uint32_t)RESET != (CAN_INTEN(can_periph) & can_interrupt_flag)){
        switch(can_interrupt_flag){
        case CAN_INT_TME:
            /* transmit mailbox empty interrupt */
            int_flag = (FlagStatus)(CAN_TSTAT(can_periph) & (CAN_TSTAT_MTF0 | CAN_TSTAT_MTF1 | CAN_TSTAT_MTF2));
            break;
        case CAN_INT_RFNE0:
            /* receive FIFO0 not empty interrupt */
            int_flag = (FlagStatus)(CAN_RFIFO0(can_periph) & CAN_RFIFO0_RFL0);
            break;
        case CAN_INT_RFF0:
            /* receive FIFO0 full interrupt */
            int_flag = (FlagStatus)(CAN_RFIFO0(can_periph) & CAN_RFIFO0_RFF0);
            break;
        case CAN_INT_RFO0:
            /* receive FIFO0 overfull interrupt */
            int_flag = (FlagStatus)(CAN_RFIFO0(can_periph) & CAN_RFIFO0_RFO0);
            break;
        case CAN_INT_RFNE1:
            /* receive FIFO1 not empty interrupt */
            int_flag = (FlagStatus)(CAN_RFIFO1(can_periph) & CAN_RFIFO1_RFO1);
            break;
        case CAN_INT_RFF1:
            /* receive FIFO1 full interrupt */
            int_flag = (FlagStatus)(CAN_RFIFO1(can_periph) & CAN_RFIFO1_RFF1);
            break;
        case CAN_INT_RFO1:
            /* receive FIFO1 overfull interrupt */
            int_flag = (FlagStatus)(CAN_RFIFO1(can_periph) & CAN_RFIFO1_RFO1);
            break;
        case CAN_INT_WERR:  
            /*  warning error interrupt */
            int_flag = (FlagStatus)(CAN_INTEN(can_periph) & CAN_INTEN_WERRIE);
            break;
        case CAN_INT_PERR:  
            /* passive error interrupt */
            int_flag = (FlagStatus)(CAN_INTEN(can_periph) & CAN_INTEN_PERRIE);
            break;
        case CAN_INT_BO:
            /* bus-off interrupt */
            int_flag = (FlagStatus)(CAN_INTEN(can_periph) & CAN_INTEN_BOIE);
            break;
        case CAN_INT_ERRN:
            /* error number interrupt */
            int_flag = (FlagStatus)(CAN_INTEN(can_periph) & CAN_ERR_ERRN);
            break;
        case CAN_INT_ERR:
            /* error interrupt */
            int_flag = (FlagStatus)(CAN_INTEN(can_periph) & CAN_STAT_ERRIF);
            break;
        case CAN_INT_WU:
            /* wakeup interrupt */
            int_flag = (FlagStatus)(CAN_STAT(can_periph) & CAN_STAT_WUIF);
            break;
        case CAN_INT_SLPW:
            /* sleep working interrupt */
            int_flag = (FlagStatus)(CAN_STAT(can_periph) & CAN_STAT_SLPIF);
            break;
        default:
            int_flag = RESET;
            break;
...
/*!
    \brief      CAN clear interrupt flag
     \param[in]  can_periph
      \arg        CANx(x=0,1)
    \param[in]  can_interrupt_flag
      \arg        CAN_INT_TME: transmit mailbox empty interrupt
...
void can_interrupt_flag_clear(uint32_t can_periph, uint32_t can_interrupt_flag)
{
    if((uint32_t)RESET != (CAN_INTEN(can_periph) & can_interrupt_flag)){
        ...
        case CAN_INT_TME:
            /* clear MTFx (rc_w1) */
            CAN_TSTAT(can_periph) |= (CAN_TSTAT_MTF0 | CAN_TSTAT_MTF1 | CAN_TSTAT_MTF2);
            break;
        case CAN_INT_RFF0:
            /* clear RFF0 (rc_w1) */
            CAN_RFIFO0(can_periph) |= CAN_RFIFO0_RFF0;
            break;
        case CAN_INT_RFO0:
            /* clear RFO0 (rc_w1) */
            CAN_RFIFO0(can_periph) |= CAN_RFIFO0_RFO0;
            break;
        case CAN_INT_RFF1:
            /* clear RFF1 (rc_w1) */
            CAN_RFIFO1(can_periph) |= CAN_RFIFO1_RFF1;
            break;
        case CAN_INT_RFO1:
            /* clear RFO1 (rc_w1) */
            CAN_RFIFO1(can_periph) |= CAN_RFIFO1_RFO1;
            break;
        case CAN_INT_WERR:
            /* clear ERRIF (rc_w1) */
        ...
        case CAN_INT_ERRN:
            /* clear ERRN bits */
            CAN_ERR(can_periph) &= ~CAN_ERR_ERRN;
            /* clear ERRIF (rc_w1) */
            CAN_STAT(can_periph) |= CAN_STAT_ERRIF;
            break;
        case CAN_INT_ERR:
        ...
        case CAN_INT_WU:
            /* clear WUIF (rc_w1) */
            CAN_STAT(can_periph) |= CAN_STAT_WUIF;
            break;
        case CAN_INT_SLPW:
            /* clear SLPIF (rc_w1) */
            CAN_STAT(can_periph) |= CAN_STAT_SLPIF;
            break;
        default:
            break;
        }
